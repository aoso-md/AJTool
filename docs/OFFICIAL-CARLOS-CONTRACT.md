# Official Carlos Contract

Source of truth for which event contracts are **confirmed by Carlos** versus
...
| Local folder / display name | Official broker ID (confirmed by Carlos) |
|---|---|
...
platform and Carlos' ecosystem refer to.

## Official flows (confirmed by Carlos)
...
These are the only two input→output pairs Carlos has confirmed as official
...
## Open items — still need Carlos' confirmation

See [`docs/OPEN-QUESTIONS-FOR-CARLOS.md`](OPEN-QUESTIONS-FOR-CARLOS.md).
...
- `CAPABILITY_ANALYSIS_COMPLETED` (calculate_cpk_ppk) has no official output
  ...
  confirmed output (`CAPABILITY_BELOW_TARGET`). Worth asking Carlos whether
